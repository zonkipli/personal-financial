...
import { Pencil, Trash2, CheckCircle, Clock, AlertTriangle, User } from "lucide-react"
  ...
  return (
    <div className="space-y-3">
      {filteredDebts.map((debt) => (
        <Card key={debt.id} className={debt.isPaid ? "opacity-60" : isOverdue(debt) ? "border-destructive" : ""}>
          <CardContent className="p-4">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="font-semibold truncate">{debt.personName}</span>
                  <Badge variant={debt.type === "receivable" ? "default" : "destructive"}>
                ...
                {debt.description && (
                  <p className="text-sm text-muted-foreground mb-2 line-clamp-2">{debt.description}</p>
                )}
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        ...
                        <AlertDialogFooter>
                          ...
                          <AlertDialogAction onClick={() => markDebtAsPaid(debt.id)}>
                            Ya, Tandai Lunas
                          </AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
