...
export function MonthlySummary({ month, year }: MonthlySummaryProps) {
  ...
  const incomeChange =
    prevStats.income > 0 ? ((income - prevStats.income) / prevStats.income) * 100 : income > 0 ? 100 : 0

  const expenseChange =
    prevStats.expense > 0 ? ((expense - prevStats.expense) / prevStats.expense) * 100 : expense > 0 ? 100 : 0
    ...
    <div className="grid gap-4 md:grid-cols-3">
      ...
      <Card>
        ...
        <CardContent>
          ...
          <div className="flex items-center gap-1 text-xs mt-1">
            ...
            <span className="text-muted-foreground">vs {getMonthName(prevMonth)}</span>
          </div>

          {topIncome.length > 0 && (
            <div className="mt-4 space-y-2">
              <p className="text-xs font-medium text-muted-foreground">Top Pemasukan</p>
              {topIncome.map((t) => (
      ...
      <Card>
        ...
        <CardContent>
          ...
          {topExpense.length > 0 && (
            <div className="mt-4 space-y-2">
              <p className="text-xs font-medium text-muted-foreground">Top Pengeluaran</p>
              {topExpense.map((t) => (
                <div key={t.id} className="flex items-center justify-between text-sm">
                  <span className="truncate">{t.description || getCategoryName(t.categoryId)}</span>
                  <span className="font-medium shrink-0">{maskAmount(formatCurrency(t.amount))}</span>
